...
export default async function InvoicesPage() {
  ...
  const cardData = await fetchCardData(); // ✅ get totals
      ...
      <table className="w-full border border-gray-300 rounded-md">
        ...
        <tbody>
          ...
          <tr className="bg-gray-50 font-semibold border-t">
            ...
            <td className="p-2">${invoices.reduce((sum, inv) => sum + inv.amount, 0)}</td>
            <td className="p-2">
              ✔️ {cardData.paidInvoices} Paid / ⏳ {cardData.pendingInvoices} Pending
            </td>
            <td className="p-2 text-gray-600">—</td>
            <td className="p-2">Total: {cardData.totalInvoices} invoices</td>
          </tr>
        </tbody>
      </table>
    </main>
